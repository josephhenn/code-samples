...
val sources = filtered.groupBy("Source")
val weightedOutDF = sources.sum("Weight")
val weightedOut = weightedOutDF.sort($"sum(Weight)".desc, $"Source")
val highestWeightedOut = weightedOut.first()

val targets = filtered.groupBy("Target")
val weightedInDF = targets.sum("Weight")
val weightedIn = weightedInDF.sort($"sum(Weight)".desc, $"Target")
val highestWeightedIn = weightedIn.first()

val sep1 = filtered.select("Source","Weight")
val sep2 = filtered.select("Target", "Weight")
val joined = sep1.union(sep2)
val nodes = joined.groupBy("Source")
val weightedTotalDF = nodes.sum("Weight")
val weightedTotal = weightedTotalDF.sort($"sum(Weight)".desc, $"Source")
val highestWeightedTotal = weightedTotal.first()
...
val header = "v,d,c"
val inDegree = highestWeightedIn.mkString(",")+",i"
val outDegree = highestWeightedOut.mkString(",")+",o"
val totalDegree = highestWeightedTotal.mkString(",")+",t"
val results = Seq(header,inDegree,outDegree,totalDegree).toDF()
